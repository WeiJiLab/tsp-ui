...
export const TaskItemCard: React.FC<PropsTypes> = (
  ...
  const renderTime = () => {
    let timeDuring = moment(task.startTime)
        .tz('Asia/ShangHai')
        .fromNow() + '-' + moment(task.endTime)
        .tz('Asia/ShangHai')
        .fromNow();
    switch (task.status) {
      case 'READY':
        ...
        <Row align={"middle"} justify={"center"}>
          <Col span={15}>
            <Row align={"middle"} justify={"center"}>
              <Col span={24}>
                <h4>
                  <FontAwesomeIcon style={{color: 'rgb(36, 66, 164)', fontSize: '1.5em'}} icon={faCubes}/>&nbsp;
                  {
                    (task.application && task.application.projectId)
                        ? <Link
                            to={'/project/' + task.application.projectId + '/' + task.application.id}>{task.application.name}
                        </Link>
                        : "None"
                  }
                </h4>
            ...
            <Row align={"middle"} justify={"center"}>
              <Col span={12}>
                <h4>
                  <FontAwesomeIcon style={{color: 'rgb(36, 66, 164)', fontSize: '1.5em'}} icon={faShieldAlt}/>&nbsp;
                  {
              ...
              <Col span={12}>
                <h4>
                  <FontAwesomeIcon style={{color: 'rgb(36, 66, 164)', fontSize: '1.5em'}} icon={faFileSignature}/>&nbsp;
                  {
          ...
          <Col span={2} style={{paddingTop: '1em'}}>
            <Link
                to={'/task/' + task.id}
            >
              <FontAwesomeIcon style={{color: 'rgb(36, 66, 164)', fontSize: '2em'}} icon={faChevronRight}/>
            </Link>
          </Col>
        </Row>
      </Card>
